...
import { tambahPegawai, updateListPegawai } from '../store/reducer/pegawaiReducer';
import store from '../store';
//INI ADALAH STATE  
const Home = () => {
    // mengambil data pegawai dari redux ( reducer pegawai )
    ...
    const [tambahId] = useState("");
    const [UbahId, setUbahId] = useState("");
    const [openConfirmDelete, setOpenConfirmDelete] = useState(false);
    const [openConfirmTambah, setOpenConfirmTambah] = useState(false);
    ...
    const getObject = async () => {
        axios({
            ...
            store.dispatch(updateListPegawai(res.data));
        })
    }
    ...
    const handleDelete = async () => {
        await axios({
            method: 'DELETE',
            url: 'https://61601920faa03600179fb8d2.mockapi.io/pegawai/' + deleteId,
        }).then((res) => {
            if (res.data) {
    ...
    const handleCreate = async () => {
        ...
        await axios({
            method: 'POST',
            url: 'https://61601920faa03600179fb8d2.mockapi.io/pegawai' + tambahId,
            data: payload,
        }).then((res) => {
            console.log(res.data);
            alert('anda telah menambah data')
            tambahPegawai({ ...payload, id: res.data.id })
            setForm({});
            setOpen(false)
            setOpenConfirmTambah(false)
            getObject();
        })
    ...
    const handleConfirmTambah = () => {

        setOpenConfirmTambah(!openConfirmTambah)
    ...
    const handleConfirmUbah = (id) => {
        setOpenConfirmUbah(!OpenConfirmUbah)
    }
    ...
    return (
        <>
            ...
            <Dialog
                //INI DIALOG ALERT UNTUK MENGHAPUS
            ...
            >
                ...
                <DialogActions>
                    <Button onClick={handleConfirmDelete}>Tidak</Button>
            ...
            <Dialog
                //INI ALERT DIALOG UNTUK MENAMBAH DATA
                open={openConfirmTambah}
                onClose={handleConfirmTambah}
                aria-labelledby="alert-dialog-title"
                aria-describedby="alert-dialog-description"
            >
                <DialogContent>
                ...
                <DialogActions>
                    <Button onClick={handleConfirmTambah}>Tidak</Button>
            ...
            <Dialog
                //INI ALERT DIALOG UNTUK MENGUBAH DATA
                open={OpenConfirmUbah}
                onClose={handleConfirmUbah}
                aria-labelledby="alert-dialog-title"
                aria-describedby="alert-dialog-description"
            >
                <DialogContent>
                ...
                <DialogActions>
                    <Button onClick={handleConfirmUbah}>Tidak</Button>
            ...
            <Button onClick={handleOpen} style={{ backgroundColor: 'blue', color: 'white', borderRadius: 100, marginLeft: 600 }} variant="outlined">
                Tambah Pegawai
            </Button>
                ...
                >
                    <div>
                        <Box style={{ width: 900, height: 700 }} sx={style}>
                            ...
                            <Grid Grid item xs={4}>
                                <IconButton style={{ backgroundColor: 'blue', marginLeft: 260, width: 160, marginTop: 300, borderRadius: 100 }} onClick={isEdit ? handleConfirmUbah : handleConfirmTambah} >
                                        ...
                                        <>
                                            <TableRow>
                                                ...
                                                <TableCell style={{ border: '1px solid #000' }}>
                                                    ...
                                                    <IconButton style={{ backgroundColor: red[500] }} onClick={() => handleConfirmDelete(v.id)}>
                                                        <DeleteIcon />
                                                    </IconButton>
                                                </TableCell>
                                            </TableRow>
